...
```bash
npx workflow-skill-pack emit-context --agent claude-code --target ~/.claude/skills --out CLAUDE.md
```
...
```bash
npx workflow-skill-pack emit-context --agent hermesagent --target ~/.hermes/skills --out HERMES.md
```
